        ...
        if os.path.exists(file_path):
            ...
            if covid_status == 'positive':
                ...
                if augment_generated < augment_count_needed:
                    ...
                    augmented_path = os.path.join(output_dir, f"{subject_id}_aug_{augment_generated + 1}.flac")
                    sf.write(augmented_path, augmented_version, sr)
                    metadata_augmented.append([f"{subject_id}_aug_{augment_generated + 1}", row['COVID_STATUS'], gender, file_type, 1])
                    augment_generated += 1
                    
                processed_count += 1
                print(f"Processed {processed_count}/{total_positive} (positive sample with augmentations).")
            else:
...
print(f"Total augmentations generated: {augment_generated}")

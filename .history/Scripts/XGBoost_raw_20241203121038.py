...
def read_data_for_ids(base_folder, ids):
    """
    Reads data for a given list of IDs from JSON files stored in a flat structure.
    """
    data = []
    for label in ['negative', 'positive']:
        for gender in ['male', 'female']:
            ...
            for id_folder in ids:
                json_path = os.path.join(label_gender_folder, f"{id_folder}.json")
                if not os.path.isfile(json_path):
                    print(f"JSON file for ID {id_folder} not found at {json_path}. Skipping.")
                    continue
                ...
                try:
                    ...
                    print(f"Error reading JSON file for ID {id_folder}: {e}. Skipping.")
                    continue

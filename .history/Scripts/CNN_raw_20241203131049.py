...
class CNNModel(nn.Module):
    def __init__(self, height, width):
        ...
        self.conv3 = nn.Conv2d(64, 128, kernel_size=(3, 3), padding=1)  # Add conv3
    ...
    def _compute_flattened_dim(self, height, width):
        ...
        x = self.pool(self.conv3(x))  # Include conv3 in flattened dim calculation
        return x.numel()

    def forward(self, x):
        x = self.pool(torch.relu(self.conv1(x)))
        x = self.pool(torch.relu(self.conv2(x)))
        x = self.pool(torch.relu(self.conv3(x)))  # Add conv3 to forward pass
        x = x.view(x.size(0), -1)
        x = self.dropout(x)

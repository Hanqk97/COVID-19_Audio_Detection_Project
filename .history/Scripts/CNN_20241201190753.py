...
def train_and_evaluate(data, model_name="CNN"):
    ...
    for train_idx, val_idx in kf.split(data):
        ...
        # Determine input dimensions from the first training sample
        sample_features = train_data[0][0]  # Extract features of the first sample
        input_length = len(sample_features)
        height = int(np.sqrt(input_length))  # Calculate height
        width = input_length // height       # Calculate width

        # Initialize the model with height and width
        model = CNNModel(height, width).to(device)  # Pass height and width explicitly

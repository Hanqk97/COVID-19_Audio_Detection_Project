...
from models import Cnn14  # PANNs CNN14 model
from torchlibrosa.augmentation import SpecAugmentation

# Initialize PANNs CNN14 model
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = Cnn14(sample_rate=16000, window_size=512, hop_size=160, mel_bins=64, fmin=50, fmax=8000, classes_num=527)
...
# Feature extraction for PANNs
def extract_panns_features(file_path, sample_rate=16000):
    ...
    with torch.no_grad():
        embeddings = model(waveform, None)
        embeddings = adaptive_avg_pool2d(embeddings, (1, 1)).squeeze()  # Global pooling
...
def extract_cough_features(file_path, sample_rate=16000):
    ...
    # PANNs embeddings
    features["panns"] = extract_panns_features(file_path).tolist()

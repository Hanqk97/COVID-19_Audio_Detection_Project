...
def read_data_for_ids(base_folder, ids):
    """
    Reads data for a given list of IDs from their JSON files.
    """
    data = []
    for id_folder in ids:
        # Search for the JSON file under the positive and negative subfolders
        json_path_positive = os.path.join(base_folder, "positive", "male", f"{id_folder}.json")
        json_path_negative = os.path.join(base_folder, "negative", "male", f"{id_folder}.json")

        json_path = (
            json_path_positive if os.path.exists(json_path_positive) else
            json_path_negative if os.path.exists(json_path_negative) else None
        )

        if not json_path:
            print(f"JSON file for ID {id_folder} not found in positive/negative folders. Skipping.")
            continue
        ...
        try:
            ...
            print(f"Error reading JSON file for ID {id_folder} at {json_path}: {e}. Skipping.")
            continue
        ...
        label_value = 1 if sample['label'] == "positive" else 0
        data.append((features, label_value))

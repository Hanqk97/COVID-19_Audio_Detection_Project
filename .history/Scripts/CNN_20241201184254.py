...
# CNN Model inspired by referenced papers
    ...
    for idx, file in enumerate(file_list):
        if file.endswith('.json'):
            with open(os.path.join(json_folder, file), 'r') as f:
                sample = json.load(f)
            features = []
            # Example: Log-Mel Spectrogram Aggregation
            features.extend(sample['cough']['log_mel'])  
            features.extend(sample['breathing']['log_mel'])
            features.extend(sample['speech']['log_mel'])
            label = 1 if sample['label'] == 'positive' else 0
            data.append((features, label))
        print(f"Read {idx + 1}/{total_files} files.", end="\r")

    ...
    for root, _, files in os.walk(base_folder):
        for file in files:
            if file.endswith(".json"):
                ...
                feature_vector = []
                # Extract features
                for key in ["mfccs_mean", "mfccs_var", "mfccs_skew", "mfccs_kurt"]:
                    feature_vector.extend(record[key])  # Extend list for array-like data
                for key in ["rms_mean", "rms_var", "zcr_mean", "zcr_var", "spectral_centroid_mean", "spectral_bandwidth_mean", "spectral_rolloff_mean", "spectral_flatness_mean", "hnr"]:
                    feature_vector.append(record[key])  # Append scalar values
...
def stratified_split_by_id(ids, labels, k_folds):
    ...
    """
    unique_ids = np.array(sorted(set(ids)))  # Sort IDs
...
def main():
    ...
    """
    input_dir = "Data/JSON"  # Folder containing JSON files

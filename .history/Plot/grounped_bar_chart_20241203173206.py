...
def plot_grouped_bar_chart(model_name, raw_f1, aug_f1, raw_auc, aug_auc, filename):
    ...
    ax.bar(x - 1.5 * width, raw_f1, width, label='F1 Score (Raw)', color='blue')
    ax.bar(x - 0.5 * width, aug_f1, width, label='F1 Score (Augmented)', color='cyan')
    ax.bar(x + 0.5 * width, raw_auc, width, label='AUC (Raw)', color='orange')
    ax.bar(x + 1.5 * width, aug_auc, width, label='AUC (Augmented)', color='red')

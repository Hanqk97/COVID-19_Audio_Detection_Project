...
augment_count_needed = total_negative - total_positive  # Required number of additional subjects
...
metadata_augmented = []
augment_generated = 0  # Track total augmented subjects
    ...
    for file_type in ['breathing', 'cough', 'speech']:
        ...
        if os.path.exists(file_path):
            ...
            if covid_status == 'positive':
                ...
                # Determine augmentation count per subject (4 initially, reduce to 3 if close to target)
                num_augmentations = 4 if (augment_generated + (total_positive * 4)) <= augment_count_needed else 3
                
                # Generate required number of augmentations for this subject
                for i in range(num_augmentations):
                    if augment_generated >= augment_count_needed:
                        break  # Stop once we reach the required augment count
                        
                    augmented_version = augment_audio(y, sr)
                    augmented_subject_id = f"{subject_id}_aug_{augment_generated + 1}"
                    ...
                    if file_type == 'speech':  # Only increment after all 3 types are done
                        augment_generated += 1

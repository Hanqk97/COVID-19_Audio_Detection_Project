...
def segment_cough(file_path, output_dir, segment_length=6, sample_rate=44100, min_cough_bursts=2):
    """
    Processes cough audio to ensure at least one 6-second segment per ID, preferably with two coughs.
    """
    ...
    if len(y) <= segment_samples:
        ...
        # Split into multiple 6-second segments
        segments = [y[i:i + segment_samples] for i in range(0, len(y), segment_samples) if len(y[i:i + segment_samples]) == segment_samples]

        # Select the most representative segment (prefers 2 cough bursts)
        best_segment = None
        max_energy = 0
        for segment in segments:
            # Calculate energy and estimate cough bursts using energy peaks
            frame_length = int(sr * 0.02)  # 20ms frame length
            hop_length = frame_length // 2
            energy = np.array([np.sum(np.abs(segment[i:i + frame_length])**2) for i in range(0, len(segment), hop_length)])
            energy_peaks = np.sum(energy > np.mean(energy))  # Count significant energy peaks

            # Prefer segments with at least `min_cough_bursts` energy peaks
            if energy_peaks >= min_cough_bursts and np.sum(segment**2) > max_energy:
                best_segment = segment
                max_energy = np.sum(segment**2)

        # Fallback to the first segment if no segment meets the criteria
        if best_segment is None:
            print(f"No valid segment with {min_cough_bursts} cough bursts found for {file_path}. Using first chunk.")
            best_segment = segments[0]

        os.makedirs(output_dir, exist_ok=True)
        segment_path = os.path.join(output_dir, f"{os.path.basename(file_path).split('.')[0]}_selected.flac")
        sf.write(segment_path, best_segment, sr)
        print(f"Saved representative cough segment: {segment_path}")
